...
class LearningRoute extends Component {
  ...
  componentDidMount() {
    LearningApiService.getHead().then(data =>
      this.setState(
        {
          head: data
        },
  ...
  handleSubmitNewWord = ev => {
    ...
    for (let i = 0; i < this.state.words.length; i++) {
      if (this.state.words[i].original === this.state.head) {
        this.setState({ currentWordIdx: i });
      }
    }
  ...
  handleSubmitCurrentWord = ev => {
    ...
    this.setState(
      { currentWord: ev.target.learn_guess_input.value },
      function() {
    ...
    if (this.state.words) {
      if (this.state.currentWordIdx < this.state.words.length) {
        ...
        document.getElementById("incorrectResponse").style.visibility =
          "visible";
  ...
  handleAnswer() {
    if (this.state.currentWordIdx < this.state.words.length) {
      return (
        <li
          className="quizWordTitle"
          key={this.state.words[this.state.currentWordIdx].id}
        >
  ...
  createWords() {
    if (this.state.currentWordIdx < this.state.words.length) {
      return (
        <li
          className="eachQuizTitle"
          key={this.state.words[this.state.currentWordIdx].id}
        >
          <h2 className="userTranslation">
            {this.state.words[this.state.currentWordIdx].original}
          </h2>
        </li>
      );
    }
    ...
    if (this.state.words) {
      if (this.state.currentWordIdx < this.state.words.length) {
        if (
          ...
          this.state.words[this.state.currentWordIdx].translation
        ) {
      ...
      <div>
        ...
        <span className="Testnextword">
          ...
          {this.state.words ? (
            this.state.currentWordIdx + 1 < this.state.words.length ? (
              <span>
                {" "}
                {this.state.words[this.state.currentWordIdx + 1].original}
              </span>
            ) : null
          ) : null}
        </span>
        ...
        <section className="correct_incorrect_count">
          <section className="thisCorrectCount">
            ...
            {this.state.words
              ? this.state.currentWordIdx < this.state.words.length
                ? this.state.words[this.state.currentWordIdx].correct_count
                : 0
              : 0}{" "}
            times.
          </section>
          ...
          <section className="thisIncorrectCount">
            ...
            {this.state.words
              ? this.state.currentWordIdx < this.state.words.length
                ? this.state.words[this.state.currentWordIdx].incorrect_count
                : 0
              : 0}{" "}
            times.
          </section>
        ...
        <form className="quiz" id="quizForm" onSubmit={this.handleSubmit}>
          ...
          <button
            ...
            onClick={() => this.setState({ newOrCurrent: "current" })}
            type="submit"
          >
            Submit your answer
          </button>
          <div className="quizWordAnswer" id="quizWordAnswer">
            <h3 className="correctPhrase" id="correctPhrase">
              {rightOrWrong !== null
                ? rightOrWrong === true
                  ? this.state.correctPhrase
                  : null
                : this.state.correctPhrase}
            </h3>
            <h3 className="incorrectPhrase" id="incorrectPhrase">
              {rightOrWrong !== null
                ? rightOrWrong === false
                  ? this.state.incorrectPhrase
                  : null
                : this.state.incorrectPhrase}
            </h3>
            <p className="correctResponse" id="correctResponse">
              {rightOrWrong !== null
                ? rightOrWrong === true
                  ? `The correct translation for ${
                      ...
                      this.state.words[this.state.currentWordIdx].translation
                    } and you chose ${this.state.currentWord}!`
                  : null
                : "The correct translation for Testnextword was test-answer-from-correct-guess and you chose test-guess-correct!"}
            </p>
            <p className="incorrectResponse" id="incorrectResponse">
              {rightOrWrong !== null
                ? rightOrWrong === false
                  ? `The correct translation for ${
                      this.state.words[this.state.currentWordIdx].original
                  ...
                  : null
                : "The correct translation for Testnextword was test-answer-from-incorrect-guess and you chose test-guess-incorrect!"}
            </p>
            <button
              formNoValidate="formNoValidate"
              onClick={() => this.setState({ newOrCurrent: "new" })}
              className="next"
              type="submit"
            >
              {this.state.words
